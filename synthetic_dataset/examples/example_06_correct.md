...
```json
{
  ...
  "rollback_procedure": {
    ...
    "steps": [
      "If connectivity issues persist after migration, revert to the previous IP address `192.168.1.100` and escalate to the network team.",
...
```json
{
  ...
  "evidence_for_error": [
    {
      ...
      "quote": "If connectivity issues persist after migration, revert to the previous IP address `192.168.1.100` and escalate to the network team."
    },
    {
      "source": "user_prompt",
  ...
  "root_cause": "The model substituted the current IP address when the 'Previous IP Address' field should have been referenced for the rollback target."
}
```
