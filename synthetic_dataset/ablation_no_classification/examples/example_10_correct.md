...
```json
{
  ...
  "borrower_options": {
    ...
    "hardship_provision": "The borrower may request to revert to the previous rate of `6.5%` within 30 days if the new rate creates financial hardship. Documentation of hardship required. If initial rate of `5.8%` is requested, a formal loan modification application must be submitted.",
...
```json
{
  "evidence_for_error": [
    {
      ...
      "quote": "The borrower may request to revert to the previous rate of `6.5%` within 30 days if the new rate creates financial hardship."
    },
    {
      "source": "user_prompt",
  ...
  "root_cause": "The model referenced the current interest rate when the 'Previous Interest Rate' field value should have been used for the reversion option."
}
```
