...
```json
{
  ...
  "contingency_clause": {
    "description": "Should budget constraints require compensation adjustments, the employee would revert to their previous salary of `$85,000` with a minimum 60-day notice period. In extreme circumstances requiring reversion to the original `$78,000` level, a formal PIP or RIF process would need to be initiated.",
...
```json
{
  "evidence_for_error": [
    {
      ...
      "quote": "Should budget constraints require compensation adjustments, the employee would revert to their previous salary of `$85,000` with a minimum 60-day notice period."
    },
    {
      "source": "user_prompt",
  ...
  "root_cause": "The model used the current salary value when populating the 'previous salary' reference in the reversion clause."
}
```
